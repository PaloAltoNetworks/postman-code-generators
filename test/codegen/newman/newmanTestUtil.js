...
function getCollections(collection_folder) {
  return fs.readdirSync(collection_folder).map((collection) => {
    return {
...
function runSnippet(testConfig, snippets, collectionName) {
  ...
  snippets.forEach((item, index) => {
    var headerSnippet = testConfig.headerSnippet
        ? testConfig.headerSnippet
        : '',
      ...
      async.waterfall(
        ...
        function (err, response) {
          ...
          if (err) {
            ...
            expect(result[0].toString().trim()).to.include(
...
module.exports = {
  ...
  runNewmanTest: function (convert, options, testConfig) {
    const collections = getCollections(PATH_TO_COLLECTIONS),
      ...
      collectionsToSkip = testConfig.skipCollections
        ? testConfig.skipCollections
        : [];

    async.eachSeries(
      collections,
      (collectionObj, callback) => {
        if (!collectionsToSkip.includes(collectionObj.name)) {
          ...
          async.map(
            ...
            function (err, snippets) {
              ...
              // Run code snippet.
              describe(
                '\nRunning newman test for: ' + collectionObj.name,
                function () {
