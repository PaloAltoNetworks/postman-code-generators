...
describe('Python- Requests converter', function () {
  ...
  it(
    'should not have allow_redirects=False twice in generated snippet when' +
      ' followRedirect option is set as false',
    function () {
  ...
  it(
    'should have correct boolean value for allow_redirects(False, uppercased F) in generated snippet when' +
      ' followRedirect option is set as false',
    function () {
      var request = new sdk.Request(mainCollection.item[0].request),
  ...
  it('should generate snippets for no files in form data', function () {
    ...
    convert(request, {}, function (error, snippet) {
      ...
      expect(snippet).to.include(
        "('no file',('file',open('/path/to/file','rb'),'application/octet-stream'))"
      );
      // eslint-disable-next-line max-len
      expect(snippet).to.include(
        "('no src',('file',open('/path/to/file','rb'),'application/octet-stream'))"
      );
      // eslint-disable-next-line max-len
      expect(snippet).to.include(
        "('invalid src',('file',open('/path/to/file','rb'),'application/octet-stream'))"
      );
    });
  });
});
