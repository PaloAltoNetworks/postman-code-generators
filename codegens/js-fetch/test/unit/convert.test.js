...
describe('js-fetch convert function for test collection', function () {
  describe('Convert function', function () {
    var request, options, snippetArray, line_no;
    ...
    it('should return a Space indented snippet ', function () {
      ...
      convert(request, options, function (error, snippet) {
        ...
        for (var i = 0; i < snippetArray.length; i++) {
          if (snippetArray[i] === 'var requestOptions = {') {
            line_no = i + 1;
          }
        }
    ...
    it(
      'should return snippet with redirect property set to manual for ' +
        'no follow redirect',
      function () {
        request = new sdk.Request(mainCollection.item[0].request);
        options = {
    ...
    it(
      'should return snippet with redirect property set to follow for ' +
        ' follow redirect',
      function () {
        request = new sdk.Request(mainCollection.item[0].request);
        options = {
    ...
    it('should trim header keys and not trim header values', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          'myHeaders.append("key_containing_whitespaces", ' +
    ...
    it('should generate snippets for no files in form data', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.be.a('string');
        expect(snippet).to.include(
          'formdata.append("no file", fileInput.files[0], "file")'
        );
        expect(snippet).to.include(
          'formdata.append("no src", fileInput.files[0], "file")'
        );
        expect(snippet).to.include(
          'formdata.append("invalid src", fileInput.files[0], "file")'
        );
      });
    });
    ...
    it('should generate valid snippet for single/double quotes in url', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.be.a('string');
        expect(snippet).to.include(
          'fetch("https://postman-echo.com/get?query1=b\'b&query2=c\\"c"'
        );
      });
    });
  });
