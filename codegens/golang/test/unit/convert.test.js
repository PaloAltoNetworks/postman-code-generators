  ...
  describe('Convert function', function () {
    ...
    it('should add content type if formdata field contains a content-type', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.contain(
          'fieldWriter1.Write([]byte("{\\"hello\\": \\"world\\"}"))'
        );
      });
    });
    ...
    it('should add time converted to seconds when input is taken in milliseconds ', function () {
      ...
      convert(request, options, function (error, snippet) {
        ...
        expect(snippet).to.include(
          'timeout := time.Duration(0.003 * time.Second)'
        );
      });
    });
    ...
    it('should trim header keys and not trim header values', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.be.a('string');
        expect(snippet).to.include(
          'req.Header.Add("key_containing_whitespaces", "  value_containing_whitespaces  ")'
        );
      });
    });
    ...
    it('should generate snippets for no files in form data', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.be.a('string');
        expect(snippet).to.include(
          'writer.CreateFormFile("no file",filepath.Base("/path/to/file"))'
        );
        expect(snippet).to.include(
          'writer.CreateFormFile("no src",filepath.Base("/path/to/file"))'
        );
        expect(snippet).to.include(
          'writer.CreateFormFile("invalid src",filepath.Base("/path/to/file"))'
        );
      });
    });
    ...
    it('should add error handling code everytime an error is possible', function () {
      ...
      requests.forEach(function (request) {
        convert(request, {}, function (error, snippet) {
          ...
          expect(snippet).to.be.a('string');
          expect(snippet.match('err := ').length).to.be.equal(
            snippet.match('if err != nil {').length
          );
        });
      });
    });
