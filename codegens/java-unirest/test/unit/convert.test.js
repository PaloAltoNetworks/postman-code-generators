var expect = require('chai').expect,
  ...
  getUrlStringfromUrlObject =
    require('../../lib/parseRequest').getUrlStringfromUrlObject,
...
describe('java unirest convert function for test collection', function () {
  describe('convert function', function () {
    ...
    it('should return a Tab indented snippet ', function () {
      ...
      convert(request, options, function (error, snippet) {
        ...
        for (var i = 0; i < snippetArray.length; i++) {
          if (
            snippetArray[i] ===
            'HttpResponse<String> response = Unirest.get("https://postman-echo.com/headers")'
          ) {
            line_no = i + 1;
          }
        }
    ...
    it(
      'should return snippet with setTimeouts function setting both ' +
        'connection and socket timeout to 0 when requestTimeout is set to 0',
      function () {
        request = new sdk.Request(mainCollection.item[0].request);
        options = {
          requestTimeout: 0,
    ...
    it(
      'should return snippet with disableRedirectHandling function for' +
        'follow redirect option set to false',
      function () {
        request = new sdk.Request(mainCollection.item[0].request);
        options = {
    ...
    it('should add content type if formdata field contains a content-type', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.contain(
          '.field("json", "{\\"hello\\": \\"world\\"}", "application/json")'
        );
      });
    });

    it(
      'should include import statements, main class and print statements ' +
        'when includeBoilerplate is set to true',
      function () {
        request = new sdk.Request(mainCollection.item[0].request);
        options = {
    ...
    it(
      'should replace propfind by default get method as unirest java only supports standard ' +
        'six HTTP methods',
      function () {
        reqObject = {
    ...
    it(
      'should not encode queryParam unresolved variables and ' +
        'leave it inside double parenthesis {{xyz}}',
      function () {
        request = new sdk.Request({
          method: 'POST',
          header: [],
        ...
        convert(request, {}, function (error, snippet) {
          ...
          expect(snippet).to.not.include(
            'http://postman-echo.com/post?a=%7B%7Bxyz%7D%7D'
          );
        });
      }
    );
    ...
    it('should trim header keys and not trim header values', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          '.header("key_containing_whitespaces", "  value_containing_whitespaces  ")'
        );
      });
    });

    it(
      'should add the force multipart body method when ' +
        'there are no files but contains text field params in multipart/form-data',
      function () {
        var request = new sdk.Request({
          method: 'GET',
          body: {
    ...
    it(
      'should not add the force multipart body method when ' +
        'there are file fields in multipart/form-data',
      function () {
        var request = new sdk.Request({
          method: 'GET',
          body: {
    ...
    it('should generate valid snippets for single/double quotes in URL', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          'Unirest.get("https://a\\"b\'c.com/\'d/\\"e")'
        );
      });
    });
  });
  describe('getUrlStringfromUrlObject function', function () {
    ...
    it(
      "should not add the auth information if user isn't present but" +
        ' password is present in the url object',
      function () {
    ...
    describe('queryParams', function () {
      ...
      it('should encode query params other than unresolved variables', function () {
        ...
        expect(outputUrlString).to.equal(
          'https://postman-echo.com/get?key=%27a%20b%20c%27'
        );
      });

      it(
        'should not encode unresolved query params and ' +
          'encode every other query param, both present together',
        function () {
          ...
          expect(outputUrlString).to.equal(
            'https://postman-echo.com/get?key1={{value}}&key2=%27a%20b%20c%27'
          );
        }
      );
