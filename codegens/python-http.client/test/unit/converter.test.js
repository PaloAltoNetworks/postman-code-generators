...
describe('Python-http.client converter', function () {
  describe('convert function', function () {
    ...
    it('should parse the url correctly even if the host and path are wrong in the url object', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          'conn.request("GET", "/path/hello", payload, headers'
        );
      });
    });
    ...
    it('should generate snippet with correct indent when body mode is formdata', function () {
      ...
      convert(
        request,
        { indentType: 'space', indentCount: 2 },
        function (error, snippet) {
          if (error) {
            expect.fail(null, null, error);
          }
    ...
    it('should generate valid snippets for single/double quotes in URL', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          'http.client.HTTPSConnection("a\\"b\'c.com")'
        );
        // expect conn.request("GET", "/'d/\"e", payload, headers)
        expect(snippet).to.include(
          'conn.request("GET", "/\'d/\\"e", payload, headers)'
        );
      });
    });
  });
  ...
  describe('parseBody function', function () {
    ...
    it('should parse body when body type is valid but body is empty', function () {
      expect(parseBody(requestEmptyFormdata.toJSON(), '\t', true))
        .to.be.a('string')
        .to.equal("boundary = ''\npayload = ''\n"); // eslint-disable-line max-len
      expect(parseBody(requestEmptyUrlencoded.toJSON(), '\t', true))
        .to.be.a('string')
        .to.equal("payload = ''\n"); // eslint-disable-line max-len
      expect(parseBody(requestEmptyRaw.toJSON(), '\t', true))
        .to.be.a('string')
        .to.equal("payload = ''\n"); // eslint-disable-line max-len
    });
  });
