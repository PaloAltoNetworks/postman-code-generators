...
describe('Request Snippet', function () {
  ...
  it('should not have script closing tag ?>', function () {
    var collection = new sdk.Collection(
        JSON.parse(
          fs
            .readFileSync('test/unit/fixtures/sample_collection.json')
            .toString()
        )
      ),
  ...
  describe('parseBody function', function () {
    it('should return empty string if request body is an empty string', function (done) {
      ...
      expect(
        parseBody(request, { indentType: 'Space', indentCount: 4 }, true)
      ).to.be.equal('');
      done();
    });
    ...
    it('should generate snippets(not error out) for requests with multiple/no file in formdata', function () {
      ...
      convert(request, {}, function (error, snippet) {
        ...
        expect(snippet).to.include(
          "array('name' => 'no file', 'type' => '<Content-type header>', 'file' => '/path/to/file', 'data' => null)"
        );
        expect(snippet).to.include(
          "array('name' => 'single file', 'type' => '<Content-type header>', 'file' => '/test1.txt', 'data' => null)"
        );
        expect(snippet).to.include(
          "array('name' => 'multiple files', 'type' => '<Content-type header>', 'file' => '/test3.txt', 'data' => null)"
        );
        expect(snippet).to.include(
          "array('name' => 'multiple files', 'type' => '<Content-type header>', 'file' => '/test3.txt', 'data' => null)"
        );
        expect(snippet).to.include(
          "array('name' => 'no src', 'type' => '<Content-type header>', 'file' => '/path/to/file', 'data' => null)"
        );
        expect(snippet).to.include(
          "array('name' => 'invalid src', 'type' => '<Content-type header>', 'file' => '/path/to/file', 'data' => null)"
        );
