...
describe('PHP HTTP_Request2 converter', function () {
  ...
  it(
    'should add code for followRedirect option when set and vice versa' +
      "(HTTP_Request2 doesn't follow redirect by default)",
    function () {
      var request = new sdk.Request(mainCollection.item[0].request);
      convert(
        request,
  ...
  it('should generate snippets for no files in form data', function () {
    ...
    convert(request, {}, function (error, snippet) {
      ...
      expect(snippet).to.include(
        "$request->addUpload('no file', '/path/to/file', 'file', '<Content-Type Header>');"
      );
      expect(snippet).to.include(
        "$request->addUpload('no src', '/path/to/file', 'file', '<Content-Type Header>');"
      );
      expect(snippet).to.include(
        "$request->addUpload('invalid src', '/path/to/file', 'file', '<Content-Type Header>');"
      );
